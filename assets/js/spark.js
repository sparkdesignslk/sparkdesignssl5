...
const wheelFarPrev = albumStage.querySelector('.wheel-far-prev img');
const wheelPrev = albumStage.querySelector('.wheel-prev img');
const wheelNext = albumStage.querySelector('.wheel-next img');
const wheelFarNext = albumStage.querySelector('.wheel-far-next img');
let albumImages = [];
let albumIndex = 0;
...
function showAlbumImage(index) {
  ...
  albumIndex = (index + albumImages.length) % albumImages.length;
  const farPrevIndex = (albumIndex - 2 + albumImages.length) % albumImages.length;
  const prevIndex = (albumIndex - 1 + albumImages.length) % albumImages.length;
  const nextIndex = (albumIndex + 1) % albumImages.length;
  const farNextIndex = (albumIndex + 2) % albumImages.length;
  wheelFarPrev.src = albumImages[farPrevIndex];
  wheelPrev.src = albumImages[prevIndex];
  lightboxImg.src = albumImages[albumIndex];
  wheelNext.src = albumImages[nextIndex];
  wheelFarNext.src = albumImages[farNextIndex];
  albumCount.textContent = `${albumIndex + 1} / ${albumImages.length}`;
}
...
function spinAlbum(direction) {
  ...
  albumStage.classList.remove('spin-up', 'spin-down');
  albumStage.classList.add(direction > 0 ? 'spin-up' : 'spin-down');
  setTimeout(() => {
    albumStage.classList.add('wheel-resetting');
    showAlbumImage(albumIndex + direction);
    albumStage.classList.remove('spin-up', 'spin-down');
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        albumStage.classList.remove('wheel-resetting');
...
function initLiquid(canvasId = 'liquidCanvas') {
