...
function showAlbumImage(index) {
  ...
  albumIndex = Math.max(0, Math.min(index, albumImages.length - 1));
  lightboxImg.src = albumImages[albumIndex];
  const thumbs = albumGallery.querySelectorAll('.album-thumb');
  thumbs.forEach((t, i) => t.classList.toggle('active', i === albumIndex));
  if (thumbs[albumIndex]) {
    thumbs[albumIndex].scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' });
  }
}
...
document.querySelectorAll('.album-card[data-images]').forEach(item => {
  item.addEventListener('click', () => {
    ...
    albumCount.textContent = item.dataset.label || `${albumImages.length} Images`;
    albumGallery.innerHTML = '';
    albumImages.forEach((src, index) => {
      const thumb = document.createElement('button');
      thumb.type = 'button';
      thumb.className = `album-thumb${index === 0 ? ' active' : ''}`;
      thumb.innerHTML = `<img src="${src}" alt="">`;
      thumb.addEventListener('click', () => showAlbumImage(index));
      albumGallery.appendChild(thumb);
    });
    showAlbumImage(0);
...
albumGallery.addEventListener('wheel', e => {
  if (!lightbox.classList.contains('open') || albumImages.length < 2) return;
  e.preventDefault();
  ...
  setTimeout(() => { albumScrollLocked = false; }, 420);
}, { passive: false });
...
document.addEventListener('keydown', e => {
  ...
  if (e.key === 'ArrowRight' || e.key === 'ArrowDown') showAlbumImage(albumIndex + 1);
  if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') showAlbumImage(albumIndex - 1);
});
