...
function applyWheelStyles(animated) {
  wheelSlots.forEach(slot => {
    ...
    const transform = [
      `translateY(${s.translateY.toFixed(1)}px)`,
...
function initLiquid(canvasId) {
  ...
  const frag = `
    ...
    void main() {
      ...
      // ── Random faint glows (6 drifting blobs, very subtle)
      float glow = 0.0;
      glow += glowBlob(uv, vec2(0.13, 0.71), 0.09, 0.28);
      glow += glowBlob(uv, vec2(0.47, 0.23), 0.07, 0.22);
      glow += glowBlob(uv, vec2(0.82, 0.55), 0.11, 0.25);
      glow += glowBlob(uv, vec2(0.31, 0.89), 0.06, 0.30);
      glow += glowBlob(uv, vec2(0.65, 0.08), 0.10, 0.20);
      glow += glowBlob(uv, vec2(0.92, 0.38), 0.08, 0.26);
      glow = clamp(glow, 0.0, 1.0);

      // Glow colour: warm deep crimson, barely visible
      vec3 glowCol = vec3(0.38, 0.0, 0.0) * glow * 0.18;
      col += glowCol;
